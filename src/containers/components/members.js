...
function Members() {
  ...
  if (getcountrys.length > 0) {
    for (let i = 0; i < getcountrys.length; i++) {
      if (
        getcountrys[i].country != null &&
        getcountrys[i].country != undefined &&
        getcountrys[i].country != "undefined"
      ) {
        country.push(
          <Option value={getcountrys[i].country}>
            {getcountrys[i].country}
          </Option>
        );
      }
    }
          ...
          <Card className={`member_card`}>
            <div className={`members_wrapper`}>
              <div className={`parent_member`}>
                {" "}
                <img
                  className={`member_logo_image`}
                  style={{ marginBottom: "8px" }}
                  src={`${hostname}/uploads/${member[i].icon}`}
                ></img>
              </div>
              <p>{member[i].name}</p>
            </div>
            <div className={`member_bottom`}>
              <p className={`click_explore`}>
                <a
                  href={`${member[i].link}`}
                  style={{ color: "black" }}
                  target="blank"
    ...
    <>
      <section id="iee433" class="flex-sect">
        <div id="ihvaeo" class="container-width">
          <div id="ih51x7" class="flex-title">
            Members
          </div>
        </div>
      </section>
      ...
      <div className={`about_members`}>
        ...
        <p>
          The association currently comprises 21 scientific partners from 14
          European countries, retaining the majority of its original founding
          members. It numbers 10 public health and 11 food/veterinary institutes
          and aims to continue to build on the success of its predecessor by
          promoting cross-disciplinary harmonisation between a more extensive
          network of veterinary and medical research institutions and
          laboratories at an EU level and beyond. This approach will help foster
          the One Heath – One Medicine agenda.
        </p>
      </div>
