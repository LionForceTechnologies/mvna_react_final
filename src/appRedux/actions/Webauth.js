import {
  ...
  export const webSignIn = ({ email, password }) => {
    return (dispatch) => {
      ...
      axios.post('http://astirs.com/auth/web_login', {
        email: email,
        password: password,
      }
      ).then((data) => {
  ...
  export const getwebpage = (data) => {
    ...
    return (dispatch) => {
      let api = `http://astirs.com/get_page/${data}`
      if(data == null){
  api = "http://astirs.com/"
      }
      axios.get(`${api}`,  {
  ...
  export const gettwitter = (data) => {
    return (dispatch) => {
      axios.get('http://astirs.com/twitter?items_per_page=10&current_page_no=').then((res) => {
        dispatch({
          type: 'gettwitters',
          payload: res.data.data
        })
